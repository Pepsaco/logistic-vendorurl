    ...
    <div class="w-full fixed left-0 right-0 top-0 px-4 mx-auto py-4 bg-white z-10 overflow-y-scroll h-full mb-100">
       <div class="grid mx-auto px-4">
           ...
           <div class="flex w-full mt-[20px]">
               <div @click="$emit('CloseDform')" class="w-[10%]">X</div>
               <span class="font-poppins text-[14px] text-center w-[90%] font-semibold">Delivery details</span>
           </div> 
       </div>
       <form action="" method="post" @submit.prevent="submitDeliverDetails" class="mb-[100px]">
...
const emit = defineEmits(["CloseDform", "adddeliveryaddress"]);
import { deliveryKey } from "@/config.js";
...
const submitDeliverDetails = () => {
   ...
   emit('adddeliveryaddress', delivery);
}

function resetForm() {
 
    let user = JSON.parse(getFromSession(deliveryKey) || '{}');
    ...
    if (user.landmark && user.landmark !== '') { user.landmark = ''; }
      ...
      saveToSession(deliveryKey, JSON.stringify(user))
    // localStorage.setItem(deliveryKey, JSON.stringify(user));
    full_name.value = ""
    phone_no.value = ""
    email.value = ""
    address.value = ""
